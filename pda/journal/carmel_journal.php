    ...
    <td>
      <center>
        <font face="Arial" size="-2">
          <strong>San Carlos Borromeo del Rio Carmelo</strong>
        </font>
      </center>
      <br>
    </td>
...
<?
  ...
  if ($HTTP_GET_VARS["id"]=="1" && $journal_name != "" && $journal_msg != "") {
    ...
    $query = "INSERT INTO carmel_journal (journal_name, journal_email, journal_msg, journal_date, user_ip) VALUES ('$new_journal_name','$new_journal_email','$new_journal_msg','".date("Y-m-d h:i:s")."','".$HTTP_SERVER_VARS['REMOTE_ADDR']."')";
    $result = mysql_query($query) or die(mysql_error());
?>
  <font face="Arial" size="+1" color="navy">Thank you! <?=$journal_name?></font><br>
  <font face="Arial" size="+1" color="navy">Your message have been successfully added to the Carmel Journal.</font><br>
  Click <a href="/journal/carmel_journal.php?pg=1">here</a> to view the Journal.
</table>
</body>
</html>
<?
...
<?
  ...
  $result = mysql_query("SELECT * FROM carmel_journal") or die(mysql_error());
  ...
  <tr>
    <td align="left">
      <font face="Arial" size="1">
        <a href="carmel_journal.htm">Add Journal Entry</a>
        &nbsp; &nbsp; &nbsp;
<?
  if($pg!=1) {
?>
        <a href="carmel_journal.php?id=0&pg=<?=($pg-1)?>">Previous</a> |
<?
  }
  ...
  for($i=1;$i<=$pages;$i++) {
    ...
    if($pages>1) {
?>
        <a href="carmel_journal.php?id=0&pg=<?=$i?>"><u><?=$i?></u></a>
<?
    }
  }
  ...
  if($pg+1<=$pages) {
?>
          <a href="carmel_journal.php?id=0&pg=<?=($pg+1)?>">Next</a>
<?}?>
      </font>
    </td>
  </tr>
<?	
  $query = "SELECT journal_name, journal_email, journal_date, journal_msg, journal_id FROM carmel_journal ORDER BY journal_date";
