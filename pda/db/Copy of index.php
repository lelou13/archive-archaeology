...
function get_select_year($name, $optselected) {
    ...
    for ($year=2000;$year<=$max_year;$year++) {
...
elseif ($edit_item_id || ($edit_item_id == "-1")) {
    ...
    elseif ($_SESSION["db_id"] == "stonetools") {
        ...
        $result = mysql_query("SELECT CatalogNumber, UnitNumber, UnitLevel, Objective, ArtifactType, DateRecovered, FSNumber, SiteName, SiteDesignation, County, State, MunsellColor, InSituCoordinateN, InSituCoordinateW, InSituCoordinateD, Description, Type, MaterialType, SpeciesIdentification, DimensionsL, DimensionsW, DimensionsD, WeightGms, DateRecorded, Recorder, Excavator, TimeModified, DateModified, UnitQuad, CrossReference, Screen, ID FROM {$_SESSION["db_id"]} WHERE ID='$edit_item_id'");
    }
...
elseif ($view_item_id) {
    if ($_SESSION["db_id"] == "garbage") {
        $result = mysql_query("SELECT CatalogNumber, TotalWeight, DateRecorded, Source, Recorder, Count, FluidOunces, SolidOunces, Cost, WasteGms, Brand, Type, Material, Totals, TimeModified, DateModified, ID FROM {$_SESSION["db_id"]} WHERE ID='$view_item_id'");
    }
    elseif ($_SESSION["db_id"] == "stonetools") {
        $result = mysql_query("SELECT CatalogNumber, UnitNumber, SizeCategory, LotCount, LotWeightGms, SpecimenType, MaterialType, CortexPercent, Description, DateRecorded, Recorder, GroupCode, TimeModified, DateModified, ID FROM {$_SESSION["db_id"]} WHERE ID='$view_item_id'");
    }
    else {
        $result = mysql_query("SELECT CatalogNumber, UnitNumber, UnitLevel, Objective, ArtifactType, DateRecovered, FSNumber, SiteName, SiteDesignation, County, State, MunsellColor, InSituCoordinateN, InSituCoordinateW, InSituCoordinateD, Description, Type, MaterialType, SpeciesIdentification, DimensionsL, DimensionsW, DimensionsD, WeightGms, DateRecorded, Recorder, Excavator, TimeModified, DateModified, UnitQuad, CrossReference, Screen, ID FROM {$_SESSION["db_id"]} WHERE ID='$view_item_id'");
    }
...
?>
    <tr>
        <td align="center"><strong>Database</strong><br><select name="db_id"><option value="carmel"<?if ($form_db_id == "carmel") {?> SELECTED<?}?>>carmel</option><option value="sjb"<?if ($form_db_id == "sjb") {?> SELECTED<?}?>>sjb</option><option value="garbage"<?if ($form_db_id == "garbage") {?> SELECTED<?}?>>garbage</option><option value="stonetools"<?if ($form_db_id == "stonetools") {?> SELECTED<?}?>>stonetools</option></select></td>
    </tr>
    <tr>
