...
if ($HTTP_POST_VARS["submit_log_in"]) {
	...
	$db_id = $HTTP_SESSION_VARS["db_id"];
}
$success = $HTTP_SESSION_VARS["success"];
session_unregister("success");
$page = ($HTTP_GET_VARS["page"] || ($HTTP_GET_VARS["page"] == "0")) ? $HTTP_GET_VARS["page"] : $HTTP_SESSION_VARS["page"];
session_register("page");
$search = $HTTP_POST_VARS["submit_search"] ? $HTTP_POST_VARS["search"] : ($HTTP_POST_VARS["submit_search_reset"] ? "" : $HTTP_SESSION_VARS["search"]);
session_register("search");
$sort = $HTTP_GET_VARS["sort"] ? $HTTP_GET_VARS["sort"] : ($HTTP_SESSION_VARS["sort"] ? $HTTP_SESSION_VARS["sort"] : "CatalogNumber");
session_register("sort");
$sort_direction = $HTTP_GET_VARS["sort_direction"] ? $HTTP_GET_VARS["sort_direction"] : ($HTTP_SESSION_VARS["sort_direction"] ? $HTTP_SESSION_VARS["sort_direction"] : "ASC");
        ...
        if (sizeof($item_sql_array)) { ### if anything requires updating
            if (!sizeof($error)) { ### if no error in validation, do db inserts/updates
                ...
                if ($form_ImageNameTmp && $form_ImageName) { ### if an image was uploaded
                    if (($form_ImageSize == 0) || ($form_ImageSize > 200000)) {
                        $error[] = "Image failed to upload!<br>Might be too large (200k max)";
                    }
                    else {
                ...
                if (!sizeof($error)) {
                    ...
                    session_register("success");
                    header("Location: $PHP_SELF?view_item_id=$edit_item_id");
                }
            }
        }
    }

    $unique_token = substr(ereg_replace("[^A-Za-z]", "", crypt(time())) . ereg_replace("[^A-Za-z]", "", crypt(time())) . ereg_replace("[^A-Za-z]", "", crypt(time())), 0, 8);
    session_register("unique_token");
?>
...
elseif ($db_id) {
    ...
    if ((($page * $page_length) + 1) > $num_rows) {
        header("Location: $PHP_SELF?page=$last_page");
    }
...
?>
    <tr>
        <td align="center"><strong>Database</strong><br><select name="db_id"><option value="carmel"<?if ($form_db_id == "carmel") {?> SELECTED<?}?>>carmel</option><option value="sjb"<?if ($form_db_id == "sjb") {?> SELECTED<?}?>>sjb</option></select></td>
    </tr>
    <tr>
